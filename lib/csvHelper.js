...
class CsvHelper {
  ...
  *processRequestData(requestFile) {
    ...
    data.forEach(function (req) {
      ...
      if (req.profile) {
        let bodyFilePath = dir + req.profile.replace('./', './');
      ...
      if (req.headers) {
        let filePath = dir + req.headers.replace('./', './');
      ...
      if (req.inputFile) {
        let bodyFilePath = dir  + req.inputFile.replace('./', './');
        console.log('bodyFilePath is ' + bodyFilePath);
        let content = utils.loadJsonOrParse(bodyFilePath);
        if (content === null) {
          content = utils.loadTextFile(bodyFilePath);
  ...
  *processValidationData(validationFile) {
  ...
  requestGenerator() {
    ...
    console.log('Writing Request Yaml file.....');
